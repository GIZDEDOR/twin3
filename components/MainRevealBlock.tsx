...
export default function MainRevealBlock() {
  ...
  const isTablet = useMediaQuery('(min-width:768px) and (max-width:1024px)', false);
...
{imagesReady && (
  <div
    ...
    className={`
      ...
      bg-gradient-to-br from-[#343434] to-[#626262]
    `}
...
<div
  ...
  style={{ 
    // для мобилки — как было,
    // для планшета (около 964px) — чуть ниже, например '30vh',
    // для десктопа — обычный текстовый top
    top: isMobile
      ? '22vh'
      : isTablet
      ? '30vh'      // <- это значение регулируйте, чтобы опустить тень
      : textPos.top,
      ...
      <div className="absolute inset-0 pointer-events-none">
        {badges.map((b, i) => (
          <div
            ...
            className="absolute inline-flex whitespace-nowrap items-center gap-2
  px-4 py-2 md:px-6 md:py-4 text-xs md:text-3xl
  rounded-2xl font-franklin font-bold text-white
