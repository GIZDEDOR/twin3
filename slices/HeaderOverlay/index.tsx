...
export default function HeaderOverlay({
  ...
  // Формируем массивы URL видео из Prismic
  const leftPool = slice.primary.left_videos
    .map((item) => (item.video as any)?.url as string)
    .filter((u): u is string => Boolean(u));

  const centerPool = slice.primary.center_videos
    .map((item) => (item.video as any)?.url as string)
    .filter((u): u is string => Boolean(u));

  const [hasMounted, setHasMounted] = useState(false);
  const [isMobile, setIsMobile] = useState(false);
  const [leftVideo, setLeftVideo] = useState(leftPool[0] || '');
  const [centerVideo, setCenterVideo] = useState(centerPool[0] || '');
  ...
  // Настройка окна и мобильной проверки
  useEffect(() => {
    setHasMounted(true);
  ...
  // Предзагрузка видео
  useEffect(() => {
    const allSources = [...leftPool, ...centerPool];
    let loadedCount = 0;
    const checkDone = () => {
      loadedCount++;
      if (loadedCount === allSources.length) {
  ...
  // CSS переменная --vh для корректной высоты
  useEffect(() => {
    const setVH = () => {
  ...
  return (
    <div
      className="fixed inset-0 z-[100] w-full h-screen sm:h-screen bg-black text-white flex flex-col sm:flex-row touch-none"
